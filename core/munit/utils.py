...
from ..datasets.kdatasets import K49
...
def get_k49_data_loaders(conf, train_version):
    batch_size = conf["batch_size"]
    num_workers = conf["num_workers"]
    data_dir = conf["data_root"]
    transform_list = [transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))]
    val_tfm = transforms.Compose(transform_list)
    transform_list = [
        transforms.ToPILImage(),
        transforms.RandomHorizontalFlip(),
    ] + transform_list
    train_tfm = transforms.Compose(transform_list)
    train_dset = K49(
        data_dir,
        conf["dataset_name"],
        f"train{train_version}",
        train_tfm,
        use_pairs=False,
        return_label=False,
    )
    val_dset = K49(
        data_dir, conf["dataset_name"], "val", val_tfm, use_pairs=False, return_label=False
    )

    class_weights = 1 / train_dset.class_counts
    sample_weights = np.array([class_weights[t] for t in train_dset.target])
    sample_weights = torch.from_numpy(sample_weights).double()
    sampler = WeightedRandomSampler(sample_weights, len(sample_weights))
    train_loader_a = DataLoader(
        train_dset, batch_size, sampler=sampler, drop_last=True, num_workers=num_workers
    )
    train_loader_b = DataLoader(
        train_dset, batch_size, sampler=sampler, drop_last=True, num_workers=num_workers
    )
    val_loader_a = DataLoader(
        val_dset, batch_size, shuffle=False, drop_last=True, num_workers=num_workers
    )
...
def create_cifar_or_gtsrb_train_and_test_datasets(conf):
    if "dataset_name" not in conf:
        raise ValueError("Dataset name is not given in configuration.")

    if conf["dataset_name"] == "GTSRB":
        dsets, _ = load_gtsrb_datasets(
            root_dir=conf["data_root"],
            alpha=None if "powerlaw_value" not in conf else conf["powerlaw_value"],
            trainer_type="munit",
            flip=conf["flip_aug"],
        )

    elif conf["dataset_name"] in ["CIFAR10", "CIFAR100"]:
        dsets, _ = load_cifar_datasets(
            root_dir=conf["data_root"],
            dataset_name=conf["dataset_name"],
            imb_type=conf["imb_type"],
            imb_factor=conf["imb_factor"],
            trainer_type="munit",
            flip=conf["flip_aug"],
        )

    train_dset_with_label = dsets["train"]
    train_dset_without_label = dsets["train_without_label"]
    val_dset = dsets["val_without_label"]

    return train_dset_with_label, train_dset_without_label, val_dset


def get_cifar_or_gtsrb_data_loaders(conf):
    _, train_dset, val_dset = create_cifar_or_gtsrb_train_and_test_datasets(conf=conf)
    batch_size = conf["batch_size"]
    num_workers = conf["num_workers"]

    sampler_a, sampler_b = None, None
    if conf["balanced_sampling"]:
        print("Balanced sampling for MUNIT.")
        sample_weights = torch.from_numpy(train_dset.sample_weights).double()
        sampler_a = WeightedRandomSampler(sample_weights, len(sample_weights))
        sampler_b = WeightedRandomSampler(sample_weights, len(sample_weights))
    train_loader_a = DataLoader(
        train_dset,
        batch_size,
        sampler=sampler_a,
...
def write_2images_modified(image_outputs, display_image_num, image_directory, postfix):
    n = len(image_outputs)
    __write_images(
        image_outputs,
        display_image_num,
        "%s/generated_%s.jpg" % (image_directory, postfix),
    )
